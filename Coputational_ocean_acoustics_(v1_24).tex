...
\textcolor{blue}{
Idealized theory on augmenting acoustic transmission across the interface
between two fluid media by quarter-wave coupling is classical \cite{a_t_a_a}.
But an appropriate value of $L$ is given by Tappert \cite{COA}
}
\begin{eqnarray}
&&k_0 L = 2
\label{eqn_1099}
...
At the approximattion region ($z_b - L / 2 < z \leq z_b + L / 2$), speed of the sound should also increase with depth.
\textcolor{blue}{
Here we use interpolation to approximate the speed of sound $c(z)$
\begin{eqnarray}
&&c(z) = c_0 \frac{z_b + L/2}{L} + c_b \frac{z_b - L/2}{L}, \hspace{0.3 cm} z_b - L / 2 < z \leq z_b + L / 2
\label{ex_12}
\end{eqnarray}
}
...
\displaystyle n^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} 
...
\textcolor{blue}{
The the sound speed at two approximate media, at air/water and water/bottom interface are
\begin{eqnarray}
&&c_{aw}(z) = c_a \frac{z_a + L_a/2}{L_a} + c_0 \frac{z_0 - L_a/2}{L_a}, \hspace{0.3 cm} z_a - L_a / 2 < z \leq z_a + L_a / 2
\label{ex_13}\\
&&c_{wb}(z) = c_0 \frac{z_b + L_a/2}{L_a} + c_b \frac{z_b - L_a/2}{L_b}, \hspace{0.3 cm} z_b - L_b / 2 < z \leq z_b + L_b / 2
\label{ex_14}
\end{eqnarray}
}
where $k_0 L_b = 2$ is an appropriate value.
\textcolor{blue}{
\cite{a_t_a_a} showed the thickness of approximate media at interface should exceed one-quarter of the wavelength. 
The difference of wavelengths at water and bottom is small, so $L_b$ can be a ratio as $2 / k_0$.
But wavelength in air is near $1/5$ as in water, so $L_a$ should be shorter.
}
The mass density of approximate media at interface is defined by (\ref{eqn_1098}) and so we can use (\ref{A16}) to find the index of refraction approximation at interface. 
...
\label{EXC_0}
\end{eqnarray}
and $n_{aw}(z) = c_0 / c_{aw}(z)$, $n_{wb}(z) = c_0 / c_{wb}(z)$ are index of refraction at two intervening medium.
The Height of $H_a$ is set over 200 wavelengths in air as 300 m.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\displaystyle n_{aw}^2(z) \left[1 + i \frac{\alpha_a^{(\lambda)}}{27.29} \right],
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\displaystyle n_{wb}^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} 
- \frac{3}{2 \rho^2} \left(\frac{\partial \rho}{\partial z} \right)^2 \right], &
...
\vskip 0cm
\hskip 4.5cm (a) \hskip 6cm (b)
\caption{(a) TL with different thicknesses of $L_a$ in the air; $L_a$ is: 0m for red line , $L_b / 3$ for black line, $L_b / 5$ for blue line, $L_b / 10$ for green line;    
...
$\rho_b = 1,500$ kg/m$^3$, $c_b = 1,590$ m/s, $\Delta r = 0.5$ m and $\Delta z = 0.5$ m, $L = 1.91$ m, $L_b = 1.9$ m 
...
the height of $H_a$ is 300m and $z_{a\max}$ is 400m.}
\label{Fig_pic12}
\end{figure}
...
\label{A16}
\end{eqnarray}
where sound speed is $c_s = \sqrt{\frac{\kappa_s}{\rho_s}}$, wave number is $k_s = \frac{\omega_s}{c_s}$.
...
\bibitem{a_t_a_a}
ACOUSTIC TRANSMISSION AUGMENTATION ACROSS AN AIR/WATER INTERFACE
Howard A. Thorpe
\end{thebibliography}
