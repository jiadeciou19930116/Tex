...
\label{EXC_0}
\end{eqnarray}
But by the derivation in appendix, we should extend $L_a$ so the variable between each grid point is small enough.(Not yet dicide the value of $L_a$)
Which mean $\rho_1$ are very small than $\rho_0$ at the interface.
}
We use the information of \cite{a_a_t_h}, choose $0^\circ C$, 10 percent relative humidity, windless air and decide.
\begin{eqnarray}
&&\tilde{n}^2(r, z) = \left\{ \begin{array}{ll}
n_a^2 \left[1 + i \frac{\alpha_a^{(\lambda)}}{27.29} \right] + 0.01 \times i \exp \left[ -\left (\frac{z_{a\max} - z}{D_a} \right )^2\right], \hspace{1in} & -z_{a\max} < z \leq -H_a \\ & \\
n_a^2 \left[1 + i \frac{\alpha_a^{(\lambda)}}{27.29} \right],
 \hspace{1in} &  -H_a < z \leq - L_a / 2 \\ & \\
n_a^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} - \frac{3}{2 \rho^2} \left(\frac{\partial \rho}{\partial z} \right)^2 \right], &
- L / 2 < z \leq 0 \\ & \\
n_0^2, \hspace{1in} & 0 < z \leq z_b - L_b / 2 \\ & \\
\displaystyle n_0^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} 
- \frac{3}{2 \rho^2} \left(\frac{\partial \rho}{\partial z} \right)^2 \right], &
...
\textcolor{blue}{
$z_{a\max}$ and $z_{b\max}$ are the up and down limitation of calculation. }
In (\ref{eqn_1092}), $D_b$ is set as one-third of thickness of ROI under sea surface; here we set $D_a = (H_a - z_{a\max}) / 3$.
\\
...
Now we consider a static solution, in which the density, the pressure, and the bulk modulus are independent of $t$ and subscript 0 for every variable. In static solution the velocity vanishes ($\bar{u}_0 = 0$) and (\ref{A2}) reduces to 
\begin{eqnarray}
&&\nabla p_0 + \rho_0 \nabla V = 0
\label{A4}
\end{eqnarray}
and $\kappa$ is defined by means of virtual nonstatic variations of the solution.
Now we add small chang,
\begin{eqnarray}
&&\rho = \rho_0 + \rho_1, \hspace{0.3 cm} \bar{u} = \bar{u}_0 + \bar{u}_1
\label{A4_1}
\end{eqnarray}
than we obtain (\ref{A1}), (\ref{A2}) and (\ref{A3}) become 
\begin{eqnarray}
&&\frac{\partial \rho_1}{\partial t} + \nabla \cdot ( \rho \bar{u}_1) = 0
\label{A5}\\
&&\rho \frac{\partial \bar{u}_1}{\partial t} + \nabla p_1 + \rho \nabla V = 0
\label{A6}\\
&&\frac{\partial p_1}{\partial t} + \bar{u}_1 \cdot \nabla p_0 = \frac{\kappa}{\rho} \left( \frac{\partial \rho}{\partial t} + \bar{u}_1  \cdot \nabla \rho \right)
\label{A7}
\end{eqnarray}
\textcolor{blue}{if $\rho_0 \gg \rho_1$ and $\kappa_0 \gg \kappa_1$ (maybe don't used with the air layer.)}, substituding (\ref{A5}) into (\ref{A7}) and we can obtain
\begin{eqnarray}
...
Here we consider $p_0$ as constanst depent on $V$, and ignored the effect gravity, then (\ref{A6}) and (\ref{A8}) are simplified as
\begin{eqnarray}
&&\rho \frac{\partial \bar{u_1}}{\partial t} + \nabla p_1 = 0
\label{A9}\\
...
\label{A11}\\
&&\nabla^2 p_1 - \frac{\rho}{\kappa_0}\frac{\partial^2 p_1}{\partial t^2} -\frac{1}{\rho} \nabla \rho \cdot \nabla p_1 = 0
\label{A12}
\end{eqnarray}
...
Then (\ref{A12}) become
\begin{eqnarray}
&&\nabla^2 P - \frac{\rho}{\kappa_0}\frac{\partial^2 P}{\partial t^2} 
+ \left[\frac{\nabla^2 \rho}{2 \rho} - \frac{3}{4} \left( \frac{\nabla \rho}{\rho} \right)^2 \right] P = 0
\label{A14}
\end{eqnarray}
