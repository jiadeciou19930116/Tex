...
changes from $\rho_1$ to $\rho_2$ and must be small compared to the vertically projected wavelength.
...
Here we use another way, the slip-step Fourier algorithm to solve this example. We apply (\ref{eqn_1088}) to find $\psi(r,z)$, and the simulation transmission loss show in Fig.(\ref{Fig_pic8}).
\textcolor{blue}{
...
; $f = 250$ Hz, $z_b = 100$ m, $z_s = 99.5$ m, $z_r = 99.5$ m, 
$\rho_a = 1.293 \times 10^{-3}$ g/cm$^3$, $c_a = 340$ m/s, $\rho_0 = 1$ g/cm$^3$, $c_0 = 1,500$ m/s, 
$\rho_b =  1.2$ g/cm$^3$, $c_b = 1,590$ m/s, $\Delta r = 1$ m and $\Delta z = 0.5$ m, $\alpha^{(\lambda)}_b = 0.5$ dB/$\lambda$, $H = 197$ m, $D = 19.67$ m, $L = 1.91$ m.}
\label{Fig_pic8}
\end{figure}
...
\vskip 5 cm
\hskip 1 cm
\special{wmf: COA_6_8_a.jpg x=6.5cm y=5.2cm}
\hskip 7 cm
\special{wmf: COA_6_8_b.jpg x=6.5cm y=5.2cm}
\vskip 0cm
\hskip 4.5cm (a) \hskip 6cm (b)
\vskip 5 cm
\hskip 1 cm
\special{wmf: COA_6_8_radia_pattern_FT_2.jpg x=6.5cm y=5.2cm}
\hskip 7 cm
\special{wmf: COA_6_8_TransLoss_30_FT_2.jpg x=6.5cm y=5.2cm}
\vskip 0cm
\hskip 4.5cm (c) \hskip 6cm (d)
\caption{(a)Test environment, (b)reference transmition loss of slip-step Fourier algorithm at 30m,
(c)$\&$(d) radiation patterns and simulate result for Fourier transform;
the up black line on (c) and (d) is interface of seawater and bottom, the down black line is the begin of absorption layer.
$f = 25$ Hz, $z_s = 100$ m, $z_r = 30$ m, 
$\rho_0 = \rho_1 = 1,000$ kg/m$^3$, $c_0 = c_1 = 1,500$ m/s, 
$\rho_b = \rho_2 = 1,500$ kg/m$^3$, $c_b = c_2 = 1,700$ m/s, $\Delta r = 1$ m and $\Delta z = 1$ m, $H = 384$ m, $D = 42.67$ m, $L = 19.1$ m, $\alpha^{(\lambda)} = \alpha_2 = 0.5$ dB/$\lambda$.}
\label{Fig_pic9}
\end{figure}
We apply Gaussian's source as the initial field.
In here, we apply (\ref{eqn_1088}) to solve another problem with a slope sea bottom.
As last example, the acceptable accuracy is $3$dB.
...
\label{EXB_1}
\end{eqnarray}
We use $\hat{\psi} = \psi/\sqrt{\rho}$ to replacing $\psi$ with the density as (\ref{eqn_1098}). 
...
In this problem, it is clear type one error is zero, and only at the interface will be error two.
...
Here are two exercises about selecting the depth of absorption layer  under seafloor and design absorption layer in the air.
...
\vskip 5 cm
\hskip 1 cm
\special{wmf: ABL_sb_all.jpg x=6.5cm y=5.2cm}
\hskip 7 cm
\special{wmf: ABL_sb_enlarge.jpg x=6.5cm y=5.2cm}
\vskip 0cm
\hskip 4.5cm (a) \hskip 6cm (b)
\vskip 5 cm
\hskip 1 cm
...
\vskip 0cm
\hskip 4.5cm (c) 
\caption{(a)TL for different depth of ABL start, 192 m for red line, 384 m for blue line and 768 m for black line (b)enlarge picture of (a),
(c)the refference solution
$f = 250$ Hz, $z_s = 99.5$ m, $z_r = 99.5$ m, 
$\rho_0 = 1,000$ kg/m$^3$, $c_0 = 1,500$ m/s, 
$\rho_b = 1,500$ kg/m$^3$, $c_b = 1,590$ m/s, $\Delta r = 0.5$ m and $\Delta z = 0.5$ m,  $L = 1.91$ m, $\alpha^{(\lambda)}_b = \alpha_2 = 0.5$ dB/$\lambda$.}
\label{Fig_pic11}
\end{figure}
...
\\
By (\ref{eqn_1098}), the mass density on air-water interface show be 
\begin{eqnarray}
&&\rho(z) = \frac{1}{2}(\rho_0 + \rho_a) + \frac{1}{2}(\rho_0 - \rho_a) \tanh\left(\frac{z - 0}{L} \right), \hspace{0.3mm}
\label{EXC_0}
\end{eqnarray}
}
We use the information of \cite{a_a_t_h}, choose $0^\circ C$, 10 percent relative humidity, windless air and decide.
\begin{eqnarray}
{\rm for}\ -H_a < z \leq -z_a , 
&&\tilde{n}^2(r, z) = n_a^2 \left[1 + i \frac{\alpha_a^{(\lambda)}}{27.29} \right] + 0.01 \times i \exp \left[ -\left (\frac{z_a - z}{D_a} \right )^2\right],
\nonumber\\
{\rm for}\ -H_a < z \leq - L / 2, 
&&\tilde{n}^2(r, z) = n_a^2 \left[1 + i \frac{\alpha_a^{(\lambda)}}{27.29} \right], 
\nonumber\\
{\rm for}\  - L / 2 < z \leq 0, 
&&\tilde{n}^2(r, z) = n_a^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} - \frac{3}{2 \rho^2} \left(\frac{\partial \rho}{\partial z} \right)^2 \right],
\nonumber\\
{\rm for}\  0 < z \leq L / 2, 
&&\tilde{n}^2(r, z) = n_0^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} - \frac{3}{2 \rho^2} \left(\frac{\partial \rho}{\partial z} \right)^2 \right],
\nonumber\\
{\rm for}\ 0 < z \leq z_b - L / 2, &&\tilde{n}^2(r, z) = n_0^2 ,
\nonumber\\
{\rm for}\ z_b - L / 2 < z \leq z_b, &&\tilde{n}^2(r, z) = n_0^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} 
- \frac{3}{2 \rho^2} \left(\frac{\partial \rho}{\partial z} \right)^2 \right],
\nonumber\\
{\rm for}\ z_b < z \leq z_b + L / 2, &&\tilde{n}^2(r, z) = n_b^2 + \frac{1}{2k_0^2}\left[ \frac{1}{\rho} \frac{\partial^2 \rho}{\partial z^2} 
- \frac{3}{2 \rho^2} \left(\frac{\partial \rho}{\partial z} \right)^2 \right],
\nonumber\\
{\rm for}\ z_b + L / 2 < z \leq H_b, &&\tilde{n}^2(r, z) = n_b^2\left[1 + i \frac{\alpha^{(\lambda)}}{27.29} \right], 
\nonumber\\
{\rm for}\ H < z \leq z_{max} , &&\tilde{n}^2(r, z) = n_b^2  \left[1 + i \frac{\alpha^{(\lambda)}}{27.29} \right] + 0.01 \times i \exp \left[ -\left (\frac{z - z_{max}}{D_b} \right )^2\right],
\label{EXC_1}
\end{eqnarray}
Where $n_a$ is the index of refraction, $\alpha_a^{(\lambda)}$ is attenuation coefficient in air \cite{a_a_t_h}; 
$z_a$ and $H_a$ are the heights of ABL up and down boundarys in air.
In (\ref{eqn_1092}), $D_b$ is set as one-third of thickness of ROI under sea surface; here we set $D_a$ as one-third of thickness of ROI above sea surface, too.
\\
The red line is no ROI in air, others are opposite.
After add the part of ROI in air, TL in all region increase about 10 dB. But the minimun happened ahead 1 km.  
...
(b) reference solution \textcolor{red}{\cite{??}};
the heights of bottom of ABL in the air are 15 m;
$f = 250$ Hz, $z_s = 99.5$ m, $z_r = 99.5$ m, 
$\rho_0 = 1,000$ kg/m$^3$, $c_0 = 1,500$ m/s, 
...
Thickness of ROI in air: 0 m for red line, 1 m for blue line, 5 m for yellow line, 10 m for green line and 50 m for black line.}
\label{Fig_pic12}
\end{figure}
...
\begin{eqnarray}
&&\frac{\partial \rho_0}{\partial t} + \nabla \cdot ( \rho_0 \bar{u}_0 ) = 0
\label{A1} \\
&&\rho_0 \frac{\partial \bar{u}_0}{\partial t} + \rho ( \bar{u}_0 \cdot \nabla ) \bar{u}_0
+ \nabla p + \rho \nabla V = 0
\label{A2} \\
&&\frac{\partial p_0}{\partial t} + \bar{u_0} \cdot \nabla p_0 = \frac{\kappa_0}{\rho_0} \frac{\partial \rho_0}{\partial t} + \bar{u}_0 \cdot \nabla \rho_0
\label{A3}
\end{eqnarray}
where $\bar{u}$ is flow velocity and $V$ is potential of gravity, $\kappa$ is bulk modulus. Subscript 0 means reference variables. 
Consider a static solution, $\rho$, $p$, $\kappa$ are independent of time, than in (\ref{A1}) velocity $\bar{u_0}$ vanishes, (\ref{A2}) reduces to 
\begin{eqnarray}
&&\nabla p_0 + \rho_0 \nabla V = 0
\label{A4}
\end{eqnarray}
We shall now consider a (non-static) solution which differs very little from the static solution considered above. the terms of subscript 1 are the only ones we shall be interested in.
We obtain (\ref{A1}), (\ref{A2}) and (\ref{A4}) become 
\begin{eqnarray}
&&\frac{\partial \rho_1}{\partial t} + \nabla \cdot ( \rho_0 \bar{u}_1 )
\label{A5}\\
&&\rho_0 \frac{\partial \bar{u}_1}{\partial t} + \nabla p_1 + \rho_1 \nabla V = 0
\label{A6}\\
&&\frac{\partial p_1}{\partial t} + \bar{u}_1 \cdot \nabla p_0 = \frac{\kappa_0}{\rho_0} \frac{\partial \rho_1}{\partial t} + \bar{u}_1  \cdot \nabla \rho_0
\label{A7}
\end{eqnarray}
substituding (\ref{A5}) into (\ref{A7}), we obtain
\begin{eqnarray}
&&\frac{\partial p_1}{\partial t} + \bar{u}_1 \cdot \nabla p_0 + \kappa_0 \nabla \cdot \bar{u}_1 = 0 
\label{A8}
\end{eqnarray}
$p_1$ is the variable commonly referred to as the sound pressure, $\rho_1$ is the mass density change of the (compressible) fluid associated with the sound wave, and $\bar{u_1}$ is the instantaneous material velocity of the fluid.
Here we consider $p_0$ as constanst depent on $V$, and ignored the effect gravity, then (\ref{A5}) and (\ref{A8}) are simplified as
\begin{eqnarray}
&&\rho_0 \frac{\partial \bar{u_1}}{\partial t} + \nabla p_1 = 0
\label{A9}\\
&&\frac{\partial p_1}{\partial t} + \kappa_0 \nabla \cdot \bar{u_1} = 0
\label{A10}
\end{eqnarray}
(\ref{A9}) is divided by $\rho_0$, (\ref{A10}) is divide by $\kappa_0$ and differentiated
with respect to the time $t$, then we obtain
\begin{eqnarray}
&&\frac{1}{\kappa_0}\frac{\partial^2 p_1}{\partial t^2} - \nabla \cdot \frac{\nabla p_1}{\rho_0} = 0
\label{A11}\\
&&\nabla^2 p_1 - \frac{\rho_0}{\kappa_0}\frac{\partial^2 p_1}{\partial t^2} -\frac{1}{\rho_0} \nabla \rho_0 \cdot \nabla p_1 = 0
\label{A12}
\end{eqnarray}
Introducing a new wave variable, namely
\begin{eqnarray}
&&P = \frac{p_1}{\sqrt{\rho_0}}
\label{A13}
\end{eqnarray}
Then (\ref{A12}) become
\begin{eqnarray}
&&\nabla^2 P - \frac{\rho_0}{\kappa_0}\frac{\partial^2 P}{\partial t^2} 
+ \left[\frac{\nabla^2 \rho_0}{2 \rho_0} - \frac{3}{4} \left( \frac{\nabla \rho_0}{\rho_0} \right)^2 \right] P = 0
\label{A14}
\end{eqnarray}
for time independent form of (\ref{A14}),
\begin{eqnarray}
&&\nabla^2 P + k_0 n^2 P + \left[ \frac{\nabla^2 \rho_0}{2 \rho_0} - \frac{3}{4} \left(\frac{\nabla \rho_0}{\rho_0}\right)^2 \right] P = 0
\label{A15} \\
&&\nabla^2 P + k_0 \tilde{n}^2 P = 0, \hspace{0.3cm} 
\tilde{n}^2 = n^2 + \left[ \frac{\nabla^2 \rho_0}{2 \rho_0} - \frac{3}{4} \left( \frac{\nabla \rho_0}{\rho_0} \right)^2 \right]
\label{A16}
\end{eqnarray}
where $k_0$ is the reference wavenumber.
The mass density approximation at interface in this paper is (\ref{eqn_1098}), here we show the first and second order partial differetial in (\ref{A16}) at bottom
\begin{eqnarray}
&&\rho(z) = \frac{\rho_b + \rho_0}{2} + \frac{\rho_b - \rho_0}{2} \tanh \left(\frac{z - z_b}{L}\right)
\\
&&\frac{\partial \rho(z)}{\partial z} = \frac{\rho_b - \rho_0}{2} \sech^2 \left(\frac{z - z_b}{L}\right)
\label{A17}\\
&&\frac{\partial^2 \rho(z)}{\partial z^2} = -\left(\rho_b - \rho_0 \right) \sech^2 \left(\frac{z - z_b}{L}\right) \tanh \left(\frac{z - z_b}{L}\right)
\label{A18}
\end{eqnarray}
